    ...
    const profile: Profile = function Profile(): boolean {
      if (iterationCounter === 0) {
        ...
        if (timestamp === 0) {
          ...
          const iterationTime_ms = Math.max(
            duration_ms / profile.iterationCount,
            0,
          );
          if (profile.bestTime > iterationTime_ms) {
            ...
            runAgain =
              profile.noImprovementCount++ < MIN_SAMPLE_COUNT_NO_IMPROVEMENT;
          }
          if (duration_ms < MIN_SAMPLE_DURATION) {
            ...
            profile.iterationCount = Math.max(
              ...
              Math.round(
                (MIN_SAMPLE_DURATION / duration_ms) * profile.iterationCount,
              ),
            );
            profile.noImprovementCount = 0;
            runAgain = true;
  ...
  benchmark.report = function (fn?: (report: string) => void) {
    ...
    (fn || console.log)(
      `Benchmark: ${benchmarkName}\n${profiles
        .map((profile: Profile) => {
          const time = (profile.bestTime * Math.pow(1000, unitOffset)).toFixed(
            3,
          );
          const percent = (
            100 -
            (profile.bestTime / fastest.bestTime) * 100
          ).toFixed(0);
          return (
            "  " +
            profile.profileName +
